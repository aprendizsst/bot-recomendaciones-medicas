...
def analizar_pdf_inteligente(texto, metadatos_pdf=None):
    ...
    for idx_l, linea in enumerate(lineas_raw):
        ...
        if formato_grilla_detectado:
            if any(stop in linea_upper for stop in ["OTRAS OBSERVACIONES", "REMISIONES:", "ATENTAMENTE", "CONSENTIMIENTO", "AUTORIZO", "TRATAMIENTO DE DATOS", "HABEAS DATA", "FIRMA DEL TRABAJADOR"]) or es_encabezado_legal(linea_limpia) or es_contenido_legal_recomendacion(linea_limpia):
                formato_grilla_detectado = False
                continue
            else:
        ...
        if any(stop in linea_upper for stop in ["OBSERVACIONES:", "OBSERVACION:", "REMISIONES:", "SISTEMA DE VIGILANCIA", "CONSENTIMIENTO", "AUTORIZO", "ATENTAMENTE"]) or es_encabezado_legal(linea_limpia) or es_contenido_legal_recomendacion(linea_limpia):
        ...
        if matched_key:
            ...
            if in_exams_section and current_exam and linea_limpia.strip():
                if not es_encabezado_legal(linea_limpia) and not es_contenido_legal_recomendacion(linea_limpia) and not (linea_limpia.isupper() and len(linea_limpia) > 10):
...
def insert_recommendations_in_placeholder(parent_container, paragraph, recom_list):
    ...
    if not combined_items:
        ...
        return
    run_first = paragraph.add_run("• " + combined_items[0]); run_first.font.name = font_name; run_first.font.size = font_size
    ...
    for item in combined_items[1:]:
        ...
        run_new = new_para.add_run("• " + item); run_new.font.name = font_name; run_new.font.size = font_size
        if color: run_new.font.color.rgb = color
        current_p = new_para
...
def generar_word_unico(datos_trabajador, lugar, fecha, template_uploaded, firma_file):
    ...
    if not consecutivo_final:
        ...
        if g_url:
            try:
                r = requests.get(g_url, params={"name": datos_trabajador["nombre"], "cargo": datos_trabajador["cargo"], "examen": datos_trabajador["tipo_examen"], "fecha": fecha.strftime("%Y-%m-%d")}, timeout=10)
    ...
    simple_replacements = {
        "{{NUMERO DE CONSECUTIVO}}": consecutivo_final, "{{TIPO DE EXAMEN}}": datos_trabajador["tipo_examen"].upper(),
        "{{LUGAR}}": lugar, "{{FECHA HOY}}": fecha.strftime("%d de %B de %Y"),
        "{{NOMBRE DE LA PERSONA}}": datos_trabajador["nombre"].upper(), "{{CARGO DE LA PERSONA}}": datos_trabajador["cargo"].upper(),
        ...
        for row in table.rows:
            for cell in row.cells:
                ...
                for idx, p in enumerate(cell.paragraphs):
                    if "VÍCTOR ALONSO MORENO CASAS" in p.text: idx_victor = idx; break
                if idx_victor != -1 and firma_file:
                    target_idx = max(0, idx_victor - 1)
                    p_firma = cell.paragraphs[target_idx]; p_firma.text = ""
...
def convertir_docx_a_pdf(docx_bytes):
    ...
    try:
        subprocess.run(["libreoffice", "--headless", "--convert-to", "pdf", "--outdir", os.path.dirname(temp_docx_path), temp_docx_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=15)
...
with col_izq:
    ...
    if pdfs_subidos:
        if len(pdfs_subidos) != st.session_state.document_count:
            st.session_state.documentos = {}; st.session_state.pdfs_raw_bytes = {}; st.session_state.zip_bytes = None; st.session_state.document_count = len(pdfs_subidos)
        for pdf in pdfs_subidos:
            if pdf.name not in st.session_state.documentos:
                pdf_raw_data = pdf.read(); st.session_state.pdfs_raw_bytes[pdf.name] = pdf_raw_data
                texto_raw = extraer_texto_pdf_robusto(pdf_raw_data); st.session_state.textos_raw[pdf.name] = texto_raw
                metadatos_pdf = extraer_metadatos_pdf_estructurados(pdf_raw_data, texto_raw)
                st.session_state.documentos[pdf.name] = analizar_pdf_inteligente(texto_raw, metadatos_pdf=metadatos_pdf)
        ...
        if archivo_seleccionado and archivo_seleccionado in st.session_state.pdfs_raw_bytes:
            st.markdown("---"); st.markdown("<h4 style='color:#60a5fa;'>📄 Soporte Visual de Comparación</h4>", unsafe_allow_html=True)
            bytes_originales = st.session_state.pdfs_raw_bytes[archivo_seleccionado]
            st.download_button(label="📥 Descargar PDF de Origen (IPS)", data=bytes_originales, file_name=f"ORIGINAL_{archivo_seleccionado}", mime="application/pdf", key="btn_download_original_source")
        ...
        archivo_seleccionado = None; st.session_state.documentos = {}; st.session_state.pdfs_raw_bytes = {}; st.session_state.zip_bytes = None; st.session_state.document_count = 0
...
with col_der:
    ...
    if archivo_seleccionado:
        if archivo_seleccionado != st.session_state.prev_colaborador:
            st.session_state.processed_doc = None; st.session_state.prev_colaborador = archivo_seleccionado
        ...
        valores_actualizados = {
            ...
            "vigilancia_programa": programa_vigilancia.strip(), "lugar": lugar, "fecha": fecha
        }
        for clave, valor in valores_actualizados.items():
            if doc_actual.get(clave) != valor: st.session_state.processed_doc = None; doc_actual[clave] = valor
            ...
            if st.button("✨ Procesar este Colaborador"):
                with st.spinner("Procesando y registrando documento..."):
                    ...
                    if "Word" in formato_salida:
                        st.session_state.processed_doc = {"bytes": bytes_word, "consec_num": consec_num, "filename": f"Informe_{nombre_persona.replace(' ','_')}.docx", "mime": "application/vnd.openxmlformats-officedocument.wordprocessingml.document"}
                    elif "PDF" in formato_salida:
                        bytes_pdf, exito = convertir_docx_a_pdf(bytes_word)
                        if exito: st.session_state.processed_doc = {"bytes": bytes_pdf, "consec_num": consec_num, "filename": f"Informe_{nombre_persona.replace(' ','_')}.pdf", "mime": "application/pdf"}
                        ...
                        st.session_state.processed_doc = {"bytes": html_out.encode('utf-8'), "consec_num": consec_num, "filename": f"Informe_{nombre_persona.replace(' ','_')}.html", "mime": "text/html"}
                st.rerun()
            if st.session_state.processed_doc is not None:
                doc_info = st.session_state.processed_doc; st.success(f"🟢 Guardado con éxito en base (Consecutivo: {doc_info['consec_num']})")
                st.download_button(label=f"📥 Descargar archivo generado ({doc_info['filename'].split('.')[-1].upper()})", data=doc_info["bytes"], file_name=doc_info["filename"], mime=doc_info["mime"])
                    ...
                    with st.spinner("Compilando lote masivo..."):
                        ...
                        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                            for filename, datos_trab in st.session_state.documentos.items():
                                ...
                                if "Word" in formato_salida: zf.writestr(f"Recomendaciones_{datos_trab['nombre'].replace(' ', '_')}.docx", bytes_word)
                                elif "PDF" in formato_salida:
                                    bytes_pdf, exito = convertir_docx_a_pdf(bytes_word)
                        ...
                        zip_buffer.seek(0); st.session_state.zip_bytes = zip_buffer.getvalue(); st.success("🎉 ZIP de lote masivo compilado con éxito."); st.rerun()
                if st.session_state.zip_bytes is not None:
                    st.download_button(label="📥 Descargar ZIP Masivo", data=st.session_state.zip_bytes, file_name=f"Lote_SST_JER_SA_{fecha.strftime('%Y%m%d')}.zip", mime="application/zip")
    else: st.markdown("<div style='text-align:center; padding: 40px; color:#64748b;'><h3>👋 Tablero Listo</h3><p>Por favor, arrastra tus archivos PDF en la sección izquierda para activar el procesamiento automático.</p></div>", unsafe_allow_html=True)
